...
mod model;
use model::{LightStatus, SimulationConfig};
...
pub async fn run_traffic_lights() -> Result<(), Box<dyn Error>> {
    ...
    // Load simulation configuration from model (base timing value)
    let sim_config: SimulationConfig = model::load_simulation_config();
