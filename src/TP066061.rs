...
mod benches {
    ...
    // Define a local version of your metrics struct for benchmarking purposes.
    #[derive(Debug, Serialize, Deserialize, Clone)]
    struct TrafficLightMetrics {
        start_time: u64,
        end_time: u64,
        total_wall_time: f64,
        total_phase_changes: u64,
        total_recommendations_received: u64,
        total_recommendations_applied: u64,
    }

    fn bench_aggregate_and_write_traffic_light_metrics(c: &mut Criterion) {
        // Simulate a vector of TrafficLightMetrics entries.
        let mut metrics_vec: Vec<TrafficLightMetrics> = Vec::new();
        for _ in 0..1000 {
            metrics_vec.push(TrafficLightMetrics {
                start_time: 1000,
                end_time: 2000,
                total_wall_time: 1000.0,
                total_phase_changes: 47,
                total_recommendations_received: 3,
                total_recommendations_applied: 2,
            });
        }
        
        c.bench_function("aggregate_and_write_traffic_light_metrics", |b| {
            b.iter(|| {
                // Aggregate the metrics similar to your simulation averages.
                let count = metrics_vec.len() as f64;
                let sum_phase_changes: u64 = metrics_vec.iter().map(|m| m.total_phase_changes).sum();
                let sum_recs_received: u64 = metrics_vec.iter().map(|m| m.total_recommendations_received).sum();
                let sum_recs_applied: u64 = metrics_vec.iter().map(|m| m.total_recommendations_applied).sum();
                let sum_wall_time: f64 = metrics_vec.iter().map(|m| m.total_wall_time).sum();

                let avg_phase_changes = sum_phase_changes as f64 / count;
                let avg_recs_received = sum_recs_received as f64 / count;
                let avg_recs_applied = sum_recs_applied as f64 / count;
                let avg_wall_time = sum_wall_time / count;

                let output = format!(
                    "--- Traffic Light Metrics Averages ---\n\
                     Average Phase Changes: {:.2}\n\
                     Average Recommendations Received: {:.2}\n\
                     Average Recommendations Applied: {:.2}\n\
                     Average Wall-Clock Time: {:.2}s\n\
                     Press Enter to return.\n\
                     -----------------------------------------------",
                    avg_phase_changes, avg_recs_received, avg_recs_applied, avg_wall_time,
                );
                // Prevent compiler optimizations on the output.
                black_box(&output);

                // Write the aggregated metrics as JSON to "traffic_light_metrics.json"
                let json_obj = serde_json::json!({
                    "average_phase_changes": avg_phase_changes,
                    "average_recommendations_received": avg_recs_received,
                    "average_recommendations_applied": avg_recs_applied,
                    "average_wall_time": avg_wall_time
                });
                let _ = fs::write("traffic_light_metrics.json", serde_json::to_string_pretty(&json_obj).unwrap());
            })
        });
    }

    criterion_group!(benches, bench_aggregate_and_write_traffic_light_metrics);
