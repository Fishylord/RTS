...
mod model;
use model::{load_simulation_config, SimulationConfig};

/// --- Configuration Constants ---
// Removed the hardcoded ACCELERATION constant; now read from SimulationConfig.
...
impl IntersectionState {
    ...
    fn process_update(&mut self, update: TrafficUpdate, acceleration: f64) -> Option<Recommendation> {
        ...
        if now < self.last_recommendation.get(&update.lane_id).unwrap_or(&0) + (300.0 / acceleration) as u64 {
            return None;
        }
        if ratio >= 1.4 {
